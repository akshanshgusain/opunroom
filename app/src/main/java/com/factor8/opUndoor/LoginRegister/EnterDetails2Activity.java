...
import com.factor8.opUndoor.databinding.ActivityEnterDetails2Binding;
    ...
    private void validate() {
         if(imageBitMap==null){
             ...
             binding.textViewError.setText("Please Select your Work Experience");
              ...
              bundle.putByteArray(ProjectConstants.INTENT_PICTURE_WORKSPACE, getFileDataFromDrawable(imageBitMap));
             bundle.putString(ProjectConstants.INTENT_FIRST_NAME_WORKSPACE, binding.editTextFirstName.getText().toString().trim());
             bundle.putString(ProjectConstants.INTENT_LAST_NAME_WORKSPACE, binding.editTextLastName.getText().toString().trim());
             bundle.putString(ProjectConstants.INTENT_PROFESSION_WORKSPACE, currentProfession);
             if(binding.editTextCurrentCompany.getText().toString().length()<=2){
                 bundle.putString(ProjectConstants.INTENT_CURRENT_COMPANY_WORKSPACE, currentCompany);
             }else{
                 bundle.putString(ProjectConstants.INTENT_CURRENT_COMPANY_WORKSPACE, binding.editTextCurrentCompany.getText().toString());
             }
             bundle.putString(ProjectConstants.INTENT_EXP_WORKSPACE, currentExp);
             Intent o  =new Intent(EnterDetails2Activity.this, TransitionActivity.class) ;
             o.putExtra(ProjectConstants.BUNDLE_REGISTER, bundle);
             startActivity(o);
         }
    }
